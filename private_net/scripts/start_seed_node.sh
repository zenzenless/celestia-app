...
echo "cp file"
cp -r /core0_dir/keyring-test "${CELESTIA_HOME}"/keyring-test

cp /core0_dir/config/*.toml "${CELESTIA_HOME}"/config/
ls -al "${CELESTIA_HOME}"/config/
cp /core0_dir/config/genesis.json "${CELESTIA_HOME}"/config/genesis.json
ls /core0_dir/
...
# wait for the node to get up and running
{
    ...
    sed -i "s/seeds = \"\"/seeds = \"${seeds}\"/g" /core0_dir/config/config.toml

    core0_peer=`cat /core0_dir/node-id`@core0:26656
    sed -i "s/persistent_peers = \"\"/persistent_peers = \"${seeds},${core0_peer}\"/g" /core0_dir/config/config.toml
    cat /core0_dir/config/config.toml
